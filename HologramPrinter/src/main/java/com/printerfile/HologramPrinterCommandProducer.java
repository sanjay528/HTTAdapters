...
public class HologramPrinterCommandProducer implements Runnable {
	...
	public HologramPrinterCommandProducer(Queue<HologramProcessRequestData> queue,PropertiesConfiguration prop, HttpClient httpClient) throws IOException {

		client = httpClient;
		lineid = prop.getString("lineid");
		this.messageQueue = queue;
		tenantIdentifier = prop.getString("tenantIdentfier");
		wait = prop.getInt("adapterwaitPeriod");
		...
		barcodeimage = prop.getString("barcodeimagelocation");
		folder = new File(barcodeimage);	
		...
		for(int i =0;i<=20;i++){
			 barcodeResult = barcodeGenerator.Genarator(messageQueue,barcodeimage,uid);
	...
	private void BarcodeGenrator() throws InterruptedException {
		logger.info(folder.list().length);
		if (folder.list().length <= 15) {
			for (int i = 0; i <= 5; i++) {
				barcodeResult = barcodeGenerator.Genarator(messageQueue,
						barcodeimage, uid);
				logger.info(barcodeResult);
				uid++;
				run();
			...
			Thread.sleep(wait);
			logger.info("thread in waiting mode");
			run();
